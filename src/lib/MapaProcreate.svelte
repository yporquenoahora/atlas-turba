<script>
    ...
    import { rutasContinentes } from "../stores/store.js";

    // convertir % a coords del viewBox 0–100
    function coordsDeHotspot(id) {
...
$:console.log($rutasContinentes)
    $: rutasDibujadas = $rutasContinentes
        .map((r) => {
            const fromPos = coordsDeHotspot(r.from);
            const toPos = coordsDeHotspot(r.to);
            if (!fromPos || !toPos) return null;

            const maxCount = Math.max(
                ...$rutasContinentes.map((rr) => rr.count || 0),
                1,
            );
            const strokeWidth = 0.4 + (r.count / maxCount) * 1.6;

            // si el ataque actual cae en esa ruta:
            let esActual = false;
            if ($ataqueActual?.ataque) {
                const fromActual = continenteDeCanal(
                    $ataqueActual.ataque.canal,
                );
                const toActual = continenteDeVictima(
                    $ataqueActual.ataque.tipo_victima,
                );
                esActual = fromActual === r.from && toActual === r.to;
            }
            ...
            return {
                ...
                strokeWidth,
                esActual,
                topMecanismo: r.topMecanismo,
                topMetafora: r.topMetafora,
            };
        })
        .filter(Boolean);
    ...
    $: continenteActivoEfectivo =
        hoveredId ?? continenteActivo ?? continenteAtaqueActualId;

    $: continenteSeleccionado = continenteActivoEfectivo
        ? continentesConfig.find((c) => c.id === continenteActivoEfectivo) ||
          null
        : null;
...
<section class="mapa-detalle">
                ...
                {:else}
                    <ul class="chips">
                        {#each topMetaforas as m}
                            <li>
                                <span>{m.nombre}</span><small>{m.valor}</small>
                            </li>
                        {/each}
                    </ul>
                {/if}
                ...
                {:else}
                    <ul class="chips">
                        {#each topVictimas as v}
                            <li>
                                <span>{v.nombre}</span><small>{v.valor}</small>
                            </li>
                        {/each}
                    </ul>
                {/if}
                ...
                {:else}
                    <ul class="chips">
                        {#each topCanales as c}
                            <li>
                                <span>{c.nombre}</span><small>{c.valor}</small>
                            </li>
                        {/each}
                    </ul>
                {/if}
    ...
    {:else}
        <p class="detalle-empty">
            Pasa el ratón por un continente o deja avanzar el replay para ver
            qué elementos contiene cada zona del mapa.
        </p>
    {/if}
</section>
...
<style>
    ...
    .ruta {
        stroke: rgba(148, 163, 184, 0.7); /* gris azulado */
        stroke-linecap: round;
        stroke-linejoin: round;
        marker-end: url(#arrowhead); /* si quieres marcador de flecha */
    }
